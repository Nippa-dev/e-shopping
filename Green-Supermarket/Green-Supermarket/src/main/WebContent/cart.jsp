...
double total = 0.00; // Initialize total to zero
if(cart_list != null) {
    ProductDao pDao = new ProductDao(DbCon.getConnection());
    cartProduct = pDao.getCartProducts(cart_list);
    ...
    // Set total as a session attribute
    session.setAttribute("totalFromCart", total);
}
%>
...
if(cart_list != null) {
	for(Cart c:cartProduct) {%>
		...
		<div class="form-group d-flex justify-content-between w-60">
		<a class="btn btn-sm btn-decre" href="quantity-inc-dec?action=dec&id=<%= c.getId()%>"><i class="fas fa-minus-square"></i></a>
		<input class="text" name="quantity" class="form-control w-60" value="<%= c.getQuantity() %>" readonly>
